...
// Customer details
    const [firstName,setFirstName]=useState(dev?'Rujal':'')
    const [lastName,setLastName]=useState(dev?'Duwal':'')
    const [email,setEmail]=useState(dev?'a@b.com':'')
    //Shipping Details
    const[shippingName,setShippingName]=useState(dev?'Rujal Duwal':'')
    const[shippingStreet,setShippingStreet]=useState(dev?'123 Fake St':'')
    const[shippingPostalZipCode,setShippingPostalZipCode]=useState(dev?'90089':'')
    ...
    const[shippingOption,setShippingOption]=useState(dev?'GB':'')
//Payment details
    const[cardNum,setCardNum]=useState(dev?'4242 4242 4242 4242':'')
    const[expMonth,setExpMonth]=useState(dev?'11':'')
    const[expYear,setExpYear]=useState(dev?'11':'')
    const[cvv,setCvv]=useState(dev?'123':'')
    const[billingPostalZipcode,setBillingPostalZipcode]=useState(dev?'90089':'')
    // Shipping and fulillment data
    ...
    const fetchShippingOptions=async (checkoutTokenId,country,stateProvince=null)=>{
        ...
        const options= await commerce.checkout.getShippingOptions(
            checkoutTokenId,{country:country,region:stateProvince}
        )
            ...
            case 1:
                return(
                    <>
                        ...
                        <FormControl className={classes.formControl}>
                            <InputLabel id="shippingCountry-label">State/Province</InputLabel>
                            <Select
    ...
    return (
        <Layout title="home" commercePublicKey={props.commercePublicKey}>
            <Typography gutterBottom vatiant="h6" color="textPrimary" component="h1">
                Checkout
            </Typography>
            {cart.loading?
                <CircularProgress/>:
                    ...
                    <Grid item md={8}>
                        <Card className={classes.p1}>
                            <form>
                                ...
                                <Box>
                                    {activeStep===steps.length? errors&& errors.length>0?(
                                        <Box>
                                            <List>
                                                {errors.map(error=>
                                                    <ListItem key={error}>
